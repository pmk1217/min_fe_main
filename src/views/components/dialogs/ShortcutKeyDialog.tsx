...
import { FlexBox } from '../CommonLayoutComponents';
import IconMouse from '../../../assets/images/icons/ic_mouse.svg?react';

const ShortcutKeyDialog = () => {
  const { confirm } = useConfirm();
  const { shortcutKeyDialog } = useDialogStore();

  return (
    <DialogLayout
      isOpen={shortcutKeyDialog.isOpen}
      title={'단축키 정리'}
      isMainDialog={true}
      isConfirm={false}
      handleClose={() => shortcutKeyDialog.actions.setIsOpen(false)}
      sx={{ width: '1200px' }}
    >
      <Box>
...
const ShorkcutKeyContainer = styled(FlexBox)({
  justifyContent: 'space-between',
  marginBottom: '7px',
});

const KeyListContainer = styled(FlexBox)({
  padding: '10px',
  alignItems: 'center',
  width: '50%',
  gap: '5px',
